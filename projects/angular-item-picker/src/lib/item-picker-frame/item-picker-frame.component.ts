...
@Component({
  selector: "app-item-picker-frame",
  templateUrl: "./item-picker-frame.component.html",
  styleUrls: ["./item-picker-frame.component.sass"]
})
export class ItemPickerFrameComponent implements OnInit {
  public filteredPickableItems: PickableItem[] = [];
  ...
  public applyFilter(event: string): void {
    if (event !== "") {
      this.filteredPickableItems = this.itemPickerData.pickerItems.filter(
        (item: PickableItem) => {
          let matched = false;
